...
int32_t Limit_32t(int32_t val,int32_t min, int32_t max)
{
...
int32_t Integral_dynamic_limit = 100;
/**
...
//	if(wProcessVarError <Integral_dynamic_limit && wProcessVarError > -Integral_dynamic_limit)
//		wProcessVarError = 0;
  ...
  {
		if(wProcessVarError <Integral_dynamic_limit && wProcessVarError > -Integral_dynamic_limit)
			wIntegral_Term = pHandle->hKiGain * wProcessVarError /40;
		else
...
//		pHandle->wIntegralTerm = Limit_32t(pHandle->wIntegralTerm,-ABS_32t(wProcessVarError * Integral_dynamic_limit),ABS_32t(wProcessVarError * Integral_dynamic_limit));
